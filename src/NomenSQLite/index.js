    ...
    static async launch(config, action, callback) {
        ...
        Object.keys(this.tables).forEach(v => {
            if (Array.isArray(this.tables[v]) && this.tables[v].length) {
                this._data[v] = [];
                this.tables[v].forEach(r => { if (typeof r.name == "string" && Object.values(this.DataType).includes(r.type)) this._data[v].push({ name: r.name, type: r.type, notnull: r.notnull || false }) })
            }
        })
        ...
        if (typeof action == "string" && action.includes('create')) {
            for (let i = 0; i < Object.keys(this._data).length; i++) {
                ...
                try { await this.instance._exec(eString); } catch (err) { errMessage.concat(err); }
            }
        }
        this.launched = true;
